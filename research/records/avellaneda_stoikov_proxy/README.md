...
## 一句话思路
A-S 做市日频代理：reservation price = 滚动 VWAP/均线参考价按 库存×gamma×日波动 调整，收盘价相对 reservation 的标准化偏离经 tanh 决定目标库存（价低建多、价高建空、偏离越大越反向），库存以 eta 速度向目标收敛并裁剪在 ±cap，权重=(库存/cap)×(1/N)；每行绝对值和 ≤ 1，行和可不为 0 但有界（净库存敞口）。
