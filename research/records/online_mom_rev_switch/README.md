...
## 一句话思路
在线动量/均值回归切换元策略：并行跟踪动量专家 ts_momentum 与均值回归专家 rsi_reversion 的零成本回测奖励，第 t 期用「截至 t-1」的 window 期奖励差均值/标准差得到相对优势 z 值，倾斜量 tilt = tilt_max·z/(1+|z|)，按 (0.5+tilt, 0.5-tilt) 在线混合两腿权重面板；预热期或优势不明时回退中性 50/50。
