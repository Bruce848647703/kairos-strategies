...
## 一句话思路
自研 2 状态高斯 EM 择时：对等权组合收益用 2 分量一维高斯混合 EM（numpy 自研、中位数分割确定性初始化、无随机数）做 walk-forward 拟合——每 refit 期只用截至当期的历史收益重拟合一次，均值较高的分量判为 bull；当期总敞口 = 当期收益属于 bull 分量的后验概率线性映射到 [bear_expo, bull_expo]，两次 refit 之间参数冻结、仅在线打分（bull 满仓、bear 空仓/降杠杆，每资产敞口/N）。
