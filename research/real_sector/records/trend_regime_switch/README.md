...
## 一句话思路
[行业均衡] 逐资产趋势/震荡状态切换：用效率比 ER（净位移/路径长度，平滑后过 er_on/er_off 双阈值滞后带状态机）判别『趋势 vs 震荡』——趋势期启用动量子逻辑（过去 mom_window 期收益为正则持有强势、为负则空仓），震荡期启用均值回归子逻辑（z-score < -entry_z 超跌买入，回归至 z > -exit_z 上方离场），两套子逻辑按状态硬切换，等预算 1/N。
