...
## 一句话思路
[行业均衡] 用收盘价近似构造 Wilder DMI/ADX：+DM=max(Δclose,0)、−DM=max(−Δclose,0)，TR=|Δclose|，DI=100×Wilder(DM)/ATR，DX=100×|+DI−(−DI)|/(+DI+(−DI))，ADX=DX 的 Wilder 平滑；仅当 ADX>阈值(趋势够强)且 +DI>−DI(方向向上)时做多，单资产仓位=等预算 1/N，逐行和≤1。
