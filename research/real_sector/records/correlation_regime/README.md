...
> 类别/途径：**dispersion** ｜ 类型：timing+sector_equalize ｜ 方向：只做多

## 一句话思路
[行业均衡] 相关性状态去杠杆：滚动 corr_window 期精确估计全部资产对的平均两两相关系数 ρ̂（E[xy]−E[x]E[y] 形式向量化，逐对 Pearson 相关取均值），把 ρ̂ 经分段线性映射为总敞口 scale ∈ [expo_min, 1]：ρ̂ ≤ rho_lo（低相关，分散化有效）满仓等权，ρ̂ ≥ rho_hi（相关性飙升，危机同步）降到 expo_min，中间线性过渡；权重 = 等权(1/N) × scale（行和 = scale ≤ 1），预热期（ρ̂ 未知）回退等权满仓。
