...
## 一句话思路
[行业均衡] 多档分位波动状态择时：对等权市场组合的已实现波动计算其在过去 lookback 期历史分布中的分位（mid-rank），以 q_low/q_mid 两个分位锚点划分低/中/高三档状态，总敞口按分位分段线性连续缩放 1.0 → scale_mid → scale_high：低波满仓、中波按比例降、高波深度降杠杆，差额即现金（每资产 scale/N）。
