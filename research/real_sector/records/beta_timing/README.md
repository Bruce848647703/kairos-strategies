...
> 类别/途径：**regime** ｜ 类型：timing+sector_equalize ｜ 方向：只做多

## 一句话思路
[行业均衡] 滚动 beta 状态择时：以等权组合为『市场』代理，逐资产滚动 window 期估计 beta（cov/var，仅用历史收益），权重乘数 g = clip(1 + k×(1 − beta), g_min, g_max)——高 beta（高风险状态）资产按比例降敞口、低 beta 资产按比例提高，每资产基础预算 1/N；等权市场的 beta 截面均值恒为 1，故行和天然 ≈ 1，超出 1 的行由 _finalize 等比缩回（组合层 ≤ 1，不加杠杆）。
