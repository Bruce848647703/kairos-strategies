...
## 一句话思路
[行业均衡] 逆波动元策略（walk-forward）：先用 engine.Backtester 对 sma_cross、donchian_turtle、ts_momentum、low_volatility、inverse_vol 各做一次零成本向量化回测得到策略收益序列，第 t 期按各基础策略「截至 t-1」的 window 期滚动波动倒数（1/σ，σ 设下限 vol_floor）加权，逐行归一到和为 1 后混合其权重面板；观测不足 min_obs 的预热期回退等权。
