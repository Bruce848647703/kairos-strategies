...
> 类别/途径：**multi_factor** ｜ 类型：cross_section+sector_equalize ｜ 方向：只做多

## 一句话思路
[行业均衡] PCA 复合因子：把四个基础因子的截面标准化值逐日堆成 N×F 矩阵，其 ZᵀZ/(N−1) 即当期因子相关阵；对截至 t−1 的滚动窗口（默认半年）平均相关阵做 numpy.linalg.eigh 特征分解，取最大特征值对应的特征向量（按 Σv ≥ 0 定向、单位化）作为复合权重，复合 alpha = 当期截面 z 向量在该方向上的投影，做多最高的约 1/3 篮子。
